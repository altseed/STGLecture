...
```RushingEnemy```は```Enemy```を基底クラスとして継承することで作成します。継承することによって、基底クラスの変数やメソッドを使いまわすことが可能です。このクラスでは基底クラスで宣言済みのPlayerクラスへの参照を示す```playerRef```並びに毎フレームごとに値が増えていくカウンタ変数```count```を使いまわします。
