        ...
        protected override void OnUpdate()
        {
            ...
            if (splitCount==count)
            {
                ...
                for(int i=0;i<6;++i)
                {
                    ...
                    dir.Degree = i * 60;
                    Layer.AddObject(new StraightMovingEnemyBullet(Position, Position + dir));
                }
...
細かく説明すると、この```SplitEnemyBullet```クラスのコンストラクタが呼ばれる前にその継承元のすべてのクラスのコンストラクタが下の図に示したように継承階層の深い順番に呼び出されます。
