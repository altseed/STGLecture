...
```SplitEnemyBullet```自体の処理としては、```count```の値を毎フレームごとに1増やし続け、```splitCount```の値と同値になった時に分裂します。分裂後の弾は360度全方向に60度の間隔で計6発出ます。そしてこの分裂前の```SplitEnemyBullet```オブジェクト自体はその瞬間に削除します。
