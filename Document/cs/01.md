...
コメントにも書いてありますが、```ace.Engine.Initialize ```でAltseedの初期化をしています。
...
ウインドウが表示されている間は、``` ace.Engine.Update(); ``` を実行しAltseedを更新します。

whileループを抜けると、その後には何もないのでAltseedの終了処理を行う必要がありますが、その時に、必ず```ace.Engine.Terminate(); ```を実行しましょう。
