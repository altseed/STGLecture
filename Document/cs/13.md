...
次は ```Background.cs``` の ```Background``` クラスにある ```OnUpdate()``` を編集しましょう。
```diff
protected override void OnUpdate()
       {
           // 毎回、Y座標をvelの値だけ位置を動かす。
...
AC-Engine では、テクスチャの色を混ぜ合わせる方法として、 ```TextureObject2D```クラス に ```AlphaBlend``` というプロパティが用意されています。```AlphaBlend```には以下のような設定ができます。
