...
以下のように ```GameScene.cs``` の ``` OnStart ```の中に以下のようにコードを追加しましょう。
...
これでループ背景がつくれそうです。まずは、 ```GameScene.cs``` の```OnStart```から編集しましょう。
...
3. ```GameScene.cs``` の ```OnStart``` で 初期位置・色・速度 の違う背景をいくつか生成する。
...
次に ```GameScene.cs``` の ```OnStart``` を編集します。速度を新しく変更しています。
