...
前章で、キャラクターが弾を撃つようになりました。しかし、ソースコードがどんどん長くなっており、段々わかりにくくなってきています。今回はそれを整理して、キャラクターと弾のソースコードを別々に管理できるようにしましょう。
...
[プロジェクト(Mac)](Projects/STG04_Mac.zip?raw=true)

[DLL(Windows)](Common/DLL.zip?raw=true)

[DLL(Mac)](Common/DLL_Mac.zip?raw=true)
...
現在はまだプレイヤーの移動と弾の発射だけですが、これから敵を加えていくと、どんどん長くなってしまいます。最終的には、開発者本人も読めなくなってしまいます。
そこで「継承」というC#の機能を使用して、ソースコードを整理していきます。
...
Mainメソッドから、インスタンスを移動させるソースコードとインスタンスが画面外に出ないようにするコードを削除します。
...
いきなり多量のソースコードを変更したため、よく分からない部分もあるかと思いますが、実行しても今までとプログラムの見た目は変わりません。ただ、whileの中が短くなりました。それでは、加えた変更について説明していきます。
...
それがこのソースコードです。``` ace.TextureObject2D ``` を元に、新しく```Player ``` というクラスを増やしています。
クラスは設計図のようなもの、と先述しましたが、設計図を元に一部を書き換えて新しい設計図を作成できます。要するに、既存のクラスを元に、新しいクラスを作成できるのです。
その設計図を元に設計図を作成することを、クラスの「継承」と呼びます。
...
ここにAC-Engineが更新された時(ace.Engine.Updateが実行された時)に実行されるメソッドが記述されています。
今回はAC-Engineが更新される度にインスタンス自身を移動させています。
...
そしてwhileループの中はAC-Engineの更新のみになりました。
...
[プロジェクト(Mac)](Projects/STG05_Mac.zip?raw=true)
