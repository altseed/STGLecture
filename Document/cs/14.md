...
当たり判定といっても、弾が当たったらHPが減るという処理を実装すれば問題ありません。HPが0になった時初めてボスを消滅させるようにしてみましょう。
...
```cs
        private void bossAroundShot()
        {
            ...
            for (int i = -nWay / 2; i <= nWay / 2; i++)
            {
                ...
                // 砲台1からの弾をレイヤーに登録する
                Layer.AddObject(new EnemyBullet(cannon1Pos, cannon1Pos + dirVector));
            }
            ...
            for (int i = -nWay / 2; i <= nWay / 2; i++)
            {
                ...
                // 砲台2からの弾をレイヤーに登録する
                Layer.AddObject(new EnemyBullet(cannon2Pos, cannon2Pos + dirVector));
            }
...
```cs
        private void bossVortexShot(float degree)
        {
            ...
            // 弾をレイヤーに登録する
            Layer.AddObject(new EnemyBullet(cannon3Pos, cannon3Pos + dirVector));
            ...
            // 弾をレイヤーに登録する
            Layer.AddObject(new EnemyBullet(cannon4Pos, cannon4Pos + dirVector));
        }
```
