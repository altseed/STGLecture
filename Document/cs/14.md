...
namespace STG
{
    public class Boss : Enemy
    {
            ...
            : base(pos, player)
        {
            ...
            // Boss のテクスチャを上書きする
            Texture = ace.Engine.Graphics.CreateTexture2D("Resources/Boss.png");
...
+        private ace.Color red = new ace.Color(255, 0, 0, 255);
...
ここで、AC-Engine が持つ ```Color``` というクラスを利用して色を設定しています。コンストラクタに渡す引数は、順番に「r値, g値, b値, アルファ値」です。最大値は255となっています。
...
ボスに当たり判定をつけてみよう…となるわけですが、その雛形はすでに ```Enemy``` クラスで実装されています。 ```Enemy``` クラスに ```ICollidable``` インターフェースを加えたことを覚えていますか？
...
このインターフェースを満たすように ```Enemy``` クラスが実装されているため、 ```Enemy``` クラスを継承した ```Boss``` クラスにも、この ```ICollidable``` インタフェースは適用されているというわけです。
