...
「プレイヤーのインスタンス」という、初めて聞く単語が出てきました。さらに、asd.TextureObject2Dやnewという単語も出てきました。それぞれについて、順を追って説明していきます。
Altseedにおいてはプログラムの中でプレイヤーや敵・弾といったゲーム中で動く物体を生成し、その物体をエンジンに追加すると、追加された物体が画面に表示されるようになっています。
その物体が「インスタンス」と呼ばれています。

さて、その物体を生成するためには、物体の設計図が必要です。asd.TextureObject2Dは、ここでいう「物体の設計図」に当たります。

また、newは「設計図から物体を生成する命令」です。

つまり、このソースコードでは、asd.TextureObject2Dという設計図からplayerという物体を生成していることになるのです。
...
asd.TextureObject2Dの中には表示される画像は設定されておらず、asd.TextureObject2Dには「画像を表示する」という機能があるだけです。そのため、外部から表示される画像を設定する必要があります。
画像を読み込むには、```asd.Engine.Graphics.CreateTexture2D``` を使用します。
今回の場合、読み込まれた画像は ```asd.TextureObject2D.Texture``` に直接設定されています。asd.TextureObject2Dのインスタンスは、設定された画像を表示します。
...
図で示すと、以下のようになります。asd.TextureObject2Dという設計図が存在し、それを元に実体を生成します。
...
asd.TextureObject2Dは、実際は予めAltseedに用意されたクラスです。
